...
# Целевая переменная: 1 — заказ доставлен позже обещанной даты, 0 — вовремя, NULL — не доставлен.
TARGET = "is_late_delivery"
# Числовые признаки: в пайплайне — заполнение пропусков медианой и стандартизация.
NUMERIC: tuple[str, ...] = (
    "items_cnt",
    "order_value",
    "freight_share",
...
# Все признаки модели в фиксированном порядке — именно эти колонки подаются в Pipeline.
...
def _bucket(order_id: str, seed: int) -> int:
    # md5 от "order_id|split|seed" → первые 8 hex-символов (32 бита) → число → остаток по корзинам.
    # Зависит только от самого order_id и seed, поэтому заказ всегда попадает в одну и ту же корзину;
    # смена seed даёт другое, но тоже воспроизводимое разбиение. md5 — не для безопасности, а
    # как быстрый равномерный хеш, одинаковый на всех платформах (в отличие от hash()).
...
def split_by_hash(
    ...
    # Стабильная сортировка по ключу: порядок строк на выходе не зависит от порядка на входе.
    ...
    # В holdout — корзины ниже порога. «Вложенность»: holdout при test_frac=0.1 целиком входит
    # в holdout при test_frac=0.2 — увеличение доли только добавляет заказы, не перемешивая старые.
...
def build_pipeline(
    ...
    """Необученный Pipeline: препроцессинг признаков + логистическая регрессия.

    Препроцессинг внутри Pipeline, поэтому его параметры (медианы, среднее/σ, словарь категорий)
    вычисляются только на данных fit — на holdout утечки нет, а на инференсе применяются те же.
    """
    # ColumnTransformer применяет свою ветку к своим колонкам и склеивает результат;
    # колонки, не перечисленные ни в одной ветке, отбрасываются.
    pre = ColumnTransformer(
        [
            # Числа: пропуски → медиана (устойчива к выбросам), затем масштаб к mean=0, std=1 —
            # иначе признаки с большими значениями (order_value, total_weight_g) доминируют в регрессии.
            (
                "num",
                Pipeline(
            ...
            # Категории: one-hot; незнакомая на инференсе категория → все нули вместо ошибки.
            ("cat", OneHotEncoder(handle_unknown="ignore"), list(categorical)),
        ]
    )
    # max_iter с запасом, чтобы solver сошёлся; class_weight="balanced" — опозданий заметно
    # меньше, чем своевременных доставок, и веса классов выравнивают вклад редкого класса.
...
def evaluate(pipe: Pipeline, x: pd.DataFrame, y: pd.Series, threshold: float) -> dict[str, float]:
    ...
    return {
        # Качество ранжирования, от порога не зависит: 0.5 — случайно, 1.0 — идеально.
        "roc_auc": float(roc_auc_score(y, proba)),
        # Площадь под precision-recall: информативнее ROC AUC при редком положительном классе.
        "pr_auc": float(average_precision_score(y, proba)),
        # Доля верных ответов при пороге threshold (единственная метрика, зависящая от порога).
        "accuracy": float(accuracy_score(y, (proba >= threshold).astype(int))),
        # Доля опозданий в выборке и её размер — контекст для интерпретации метрик.
...
def dataset_fingerprint(df: pd.DataFrame, params: dict[str, Any]) -> str:
    ...
    # sort_keys — одинаковый JSON при любом порядке ключей; default=str — для дат, Path и т.п.
