...
Граф ML-ассетов (группа "ml"):
    ...
    mart_order_features (dbt)  →  training_dataset  →  model  →  model_evaluation
                                        ↑ check no_leakage             ↑ check quality_gate (blocking)

Вся логика обучения — чистые функции в olist_ml.features; здесь только Dagster-обвязка:
...
# Абсолютный путь к DuckDB из .env; импорт заодно гарантирует, что окружение загружено.
from olist_ml.defs.env import DUCKDB_PATH

# Та же БД, куда пишет dbt (dbt/profiles.yml, таргет duck); путь — из .env (defs/env.py).
# Схема marts задаётся в dbt_project.yml (+schema: marts), имя таблицы = имя dbt-модели.
...
@dg.asset(
    ...
    # Зависимость без передачи данных: Dagster знает, что ассет строится после dbt-модели
    # (и материализует её раньше в общем запуске), а сами данные читаем из DuckDB ниже.
...
def training_dataset() -> dg.Output[dict]:
    ...
    # read_only: не блокируем файл БД для записи — dbt может работать параллельно.
    ...
    # Таргет NULL у ещё не доставленных заказов: для обучения и оценки они не годятся.
    df = mart_order_features[mart_order_features[F.TARGET].notna()]
    # 20% в holdout; сплит по хешу order_id — воспроизводим при любом порядке строк.
    train, holdout = F.split_by_hash(df, test_frac=0.2, seed=42)
    # Значение ассета (dict с двумя DataFrame) сохраняет IO manager (по умолчанию pickle
    # в DAGSTER_HOME/storage) и передаёт в зависимые ассеты; metadata видна в UI.
    return dg.Output(
...
# code_version: при изменении логики обучения увеличить — Dagster пометит модель устаревшей.
...
def model(training_dataset: dict):
    ...
    # Аргумент training_dataset — значение одноимённого ассета (Dagster связывает их по имени).
    train = training_dataset["train"]
    pipe = F.build_pipeline(random_state=42)
    # Препроцессинг внутри Pipeline обучается тоже только на train — holdout не «подглядывает».
    # Таргет может прийти как float/nullable int после pandas — приводим к int для sklearn.
...
def model_evaluation(
    ...
    # threshold=0.5 влияет только на accuracy; ROC AUC и PR AUC от порога не зависят.
    m = F.evaluate(
        model, h[list(F.FEATURES)], h[F.TARGET].astype(int), threshold=0.5
    )
    # Сам словарь метрик — значение ассета (его читает quality_gate); округлённые копии —
    # в metadata, чтобы видеть динамику метрик от запуска к запуску.
    return dg.Output(
        m,
        metadata={
...
# target — очевидно; delivery_delay_days и дата доставки — известны только после доставки;
# review_score — отзыв ставят после получения заказа.
LEAKY = {
    F.TARGET,
    "delivery_delay_days",
...
def no_leakage(training_dataset: dict) -> dg.AssetCheckResult:
    # Проверяем контракт признаков (F.FEATURES), а не колонки датасета: в датасете эти поля
    # могут быть (для анализа), важно лишь, чтобы модель их не использовала.
...
# Параметры проверки задаются в run config при запуске (Launchpad в UI), без правки кода.
...
@dg.asset_check(
    ...
    # blocking: если проверка не прошла, ассеты ниже по графу в этом запуске не строятся.
    blocking=True,
...
def quality_gate(
    ...
    return dg.AssetCheckResult(
        ...
        # ERROR (а не WARN): провал — красный статус в UI и срабатывание blocking.
